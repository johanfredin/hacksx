...
void tf_add_layers_to_frame(Frame *frame, GsSPRITE *tile_set, Tile_Map *map) {
    Tile_Layer *tl_curr;
    SpriteLayer **bg_layers = NULL, **fg_layers = NULL;
    u_char n_layers_bg = 0, n_layers_fg = 0, idx_bg = 0, idx_fg = 0;
    u_short i, n_layers_total = 0;
    SpriteLayer **sprite_layers = MEM_CALLOC_3_PTRS(map->layers_cnt, SpriteLayer);
    ...
    for (tl_curr = map->layers; tl_curr != NULL; tl_curr = tl_curr->next) {
        ...
        if (sl->type == NULL) {
            ...
            exit(1);
        } else if(STREQ(sl->type, "bg")) {
            n_layers_bg++;
        } else if(STREQ(sl->type, "fg")) {
            n_layers_fg++;
        } else {
        ...
        sprite_layers[n_layers_total++] = sl;
    }

    // Split into sub-groups for bg and fg layers
    bg_layers = MEM_CALLOC_3_PTRS(n_layers_bg, SpriteLayer);

    if(n_layers_fg > 0) {   // Fg layers not required
        fg_layers = MEM_CALLOC_3_PTRS(n_layers_fg, SpriteLayer);
    }
